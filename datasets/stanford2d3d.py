...
class Stanford2D3D(Dataset):
    def __init__(self, filelist_path, mode, size=(560, 1120), color_aug=False, LR_filp_aug=False, yaw_rot_aug=False):
    ...
    def __getitem__(self, item):
        ...
        sample = self.transform({'image': eq_image, 'depth':depth})
        ...
        aug_cb_image = self.e2c.run(eq_image.transpose(1,2,0))
        ...
        sample['valid_mask'] = ((sample["depth"] > 0) & (sample["depth"] <= self.max_depth_meters)
                                & ~torch.isnan(sample["depth"]))
